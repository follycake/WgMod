...
{
    /// <summary> The head segment for the worm.  Only one "head" is considered to be active for any given worm </summary>
    Head,
    /// <summary> The body segment.  Follows the segment in front of it </summary>
    Body,
    /// <summary> The tail segment.  Has the same AI as the body segments.  Only one "tail" is considered to be active for any given worm </summary>
    Tail
}

/// <summary> The base class for non-separating Worm enemies.  </summary>
public abstract class Worm : ModNPC
{
    ...
    /// <summary> The rate at which the NPC gains velocity </summary>
    public float Acceleration { get; set; }

    /// <summary> The NPC instance of the head segment for this worm. </summary>
    public NPC HeadSegment => Main.npc[NPC.realLife];

    /// <summary> The NPC instance of the segment that this segment is following (ai[1]).  For head segments, this property always returns <see langword="null"/>. </summary>
...
/// <summary> The base class for head segment NPCs of Worm enemies </summary>
public abstract class WormHead : Worm
{
    ...
    /// <summary> The minimum amount of segments expected, including the head and tail segments </summary>
    public int MinSegmentLength { get; set; }

    /// <summary> The maximum amount of segments expected, including the head and tail segments </summary>
    ...
    /// <summary> Whether the NPC uses </summary>
    public virtual bool HasCustomBodySegments => false;

    /// <summary> If not <see langword="null"/>, this NPC will target the given world position instead of its player target </summary>
    ...
    /// <summary> Spawns a body or tail segment of the worm. </summary>
    ...
    bool HeadAI_CheckCollisionForDustSpawns()
    {
    ...
    void HeadAI_CheckTargetDistance(ref bool collision)
    {
    ...
    void HeadAI_Movement(bool collision)
    {
    ...
    void HeadAI_Movement_HandleFallingFromNoCollision(float dirX, float speed, float acceleration)
    {
    ...
    void HeadAI_Movement_PlayDigSounds(float length)
    {
        if (NPC.soundDelay == 0)
        {
    ...
    void HeadAI_Movement_HandleMovement(float dirX, float dirY, float length, float speed, float acceleration)
    {
    ...
    void HeadAI_Movement_SetRotation(bool collision)
    {
...
public abstract class WormBody : Worm
{
    ...
    internal static void CommonAI_BodyTail(Worm worm)
    {
        ...
        if (following is not null)
        {
            ...
            float length = (float)Math.Sqrt(dirX * dirX + dirY * dirY) * 1.1f;
