...
public class GorgeistBossBody : ModNPC
{
	...
	*/
	...
	void SpawnMinions()
	{
		...
		GorgeistMaxHealthTotal = 0;
		for (int i = 0; i < count; i++)
		{
			NPC minionNPC = NPC.NewNPCDirect(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<HomingFood>(), NPC.whoAmI);
	...
	void CheckSecondStage()
	{
		...
		if (GorgeistHealthTotal <= 0 && Main.netMode != NetmodeID.MultiplayerClient)
		{
	...
	void DoFirstStage(Player player)
	{
	...
	void ApplySecondStageBuffImmunities()
	{
